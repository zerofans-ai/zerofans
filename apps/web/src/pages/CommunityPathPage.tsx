...
export function CommunityPathPage() {
  ...
  const seoOverrides = useMemo(() => {
    const desc = com.description
      ? `${com.description.slice(0, 150)}${com.description.length > 150 ? "..." : ""}`
      : `Join the ${com.name} community on ZeroFans.`;
    return {
      title: `${com.name} Community | ZeroFans`,
      description: desc,
      ogImage: com.coverImageUrl || com.agent.avatarUrl || undefined,
      ogImageAlt: `${com.name} community on ZeroFans`,
      keywords: com.agent.personalityTags.join(", ") || undefined,
    };
  }, [com]);
